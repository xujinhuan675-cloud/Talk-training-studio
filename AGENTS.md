...
- 本地工作区不创建数据库、配置或二进制的备份副本。配置变更通过事务、迁移、Git 和非敏感变更记录保持可审计、可重放；没有有效凭据时，不创建伪装成可用的占位渠道。
...
4. 停止第 3 步确认的目标 PID，把 `new-api-talkwise-next.exe` 原位替换为 `new-api-talkwise.exe`；不备份旧二进制。
...
本地 NewAPI 根目录只保留当前统一运行入口 `new-api-talkwise.exe`。验收通过后删除临时构建，不保留带日期、`previous`、`restart`、`bak`、`backup` 等标记的历史应用程序或数据库副本。
