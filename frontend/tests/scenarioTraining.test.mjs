...
test('scenario prompts carry realistic customer simulation rules', () => {
  ...
  assert.match(drillPrompt, /guidance panel handles that separately/)
  assert.match(drillPrompt, /Do not include correction, rewrite guidance/)
  assert.doesNotMatch(drillPrompt, /one concise correction, one stronger rewrite/)
  assert.doesNotMatch(drillPrompt, /give coaching advice/)
  assert.match(drillPayload.style, /natural customer reply/)
  assert.match(drillPayload.style, /product shows guidance separately/)
