...
function visibleUsersForViewer(users: AuthUser[], currentUser: AuthUser | null): AuthUser[] {
  ...
  if (currentUser.systemRole === 'staff') return users.filter((user) => user.userId === currentUser.userId)
...
export default function ScenarioLeaderboardPage() {
  ...
  const selectedUser = visibleAuthUsers.find((user) => user.userId === selectedUserId)
    ?? currentUser
    ?? visibleAuthUsers[0]
    ?? null
      ...
      <section className="scenario-leaderboard-personal">
        ...
        {personal ? (
          <>
            ...
            {personal.status === 'partial' && (
              <div className="scenario-leaderboard-banner">
                <AlertCircle size={16} />
                还有 {personal.totalRequired - personal.completedRequired} 个必练场景未完成：
                {personal.unfinishedRequired.map((scenario) => scenario.title).join('、')}
              </div>
            )}
